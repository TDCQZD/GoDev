...
func (this *UserProcess)ServerProcessLogout(mes *message.Message) (err error) {
	...
	var logout message.LoginOut
	err = json.Unmarshal([]byte(mes.Data), &logout) 
	if err != nil {
		fmt.Println("json.Unmarshal fail err=", err)
		return 
	}	
	this.NotifyOthersOnoffUser(logout.UserId)
	
	return
}

func (this *UserProcess) NotifyOthersOnoffUser(userId int) {
	...
	for id, up := range userMgr.onlineUsers {
		...
		//开始通知【单独的写一个方法】
		up.NotifyMeOnline(userId,message.UserOffline)
	}
}
...
func (this *UserProcess) NotifyOthersOnlineUser(userId int) {
	...
	for id, up := range userMgr.onlineUsers {
		...
		//开始通知【单独的写一个方法】
		up.NotifyMeOnline(userId,message.UserOnline)
	}
}

func (this *UserProcess) NotifyMeOnline(userId int,statue int) {
	...
	notifyUserStatusMes.Status = statue
